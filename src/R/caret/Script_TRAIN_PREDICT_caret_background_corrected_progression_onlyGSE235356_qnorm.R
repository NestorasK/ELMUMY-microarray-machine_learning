...
preds_out <- foreach(j = seq_len(length.out = length(files_expr_stable))) %do% {
    ...
    preds_all_transform <- foreach(
        ...
        rownames(train_x) <- expr_t_meta[-unlist(ncvfolds), c(rn)]
        ...
        preds_transformj <- lapply(
            ...
            FUN = function(mli) {
                dt_mli <- data.table(
                    predict.train(
                        object = mli, newdata = holdout_x_qnorm,
                        type = "prob"
                    )
                )
                dt_mli$method <- mli$method
                dt_mli$rn <- rownames(holdout_x_qnorm)
                return(dt_mli)
            }
        ) %>% rbindlist()
...
fwrite(
    x = preds_all,
    file = paste0(path2save, "predictions_optimizing_auc/predictions_qnorm.csv")
)
